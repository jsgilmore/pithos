...
\begin{tabular}{|c|l|l|}
\hline
Storage method & Reliability & Responsiveness \\
               &             & mean (var.) (s) \\
\hline
Safe    &  0.9705  &   1.554 (3.178)  \\
Fast    &  1.0     &   0.0488 (0.0046) \\
\hline
\end{tabular}
...
\end{figure}
%
Figure \ref{fig_group_get_sf} shows group retrieval responsiveness for fast retrieval. This type of retrieval has 24\% of requests with zero second latency, due to nodes requesting objects from the network and those objects being found in the locally stored root object store. Because of the high number of replicas compared to the average group size used in the simulation, many of the requests can be locally served, which greatly increases responsiveness.

It should be noted that the node that originally stored the object does not have local access to the object. Only other nodes that did not originally store the object may have access to local copies. That is the reason why no such spike is present in fast storage.
...
\begin{figure}[htbp]
 \centering
 \includegraphics[clip=true, viewport=10mm 45mm 275mm 150mm, width=\columnwidth]{repair_bw}
