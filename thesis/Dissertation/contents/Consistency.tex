\chapter{State Consistency in Virtual Environments}
...
It should be noted that a peer does not have to request object state every time it wishes to render it. A peer will render the object according to the local state of the object. It might be possible that a local object's state has not yet been updated with a state update that is en route. To maintain interactivity in the virtual environment, all operations should occur on the current state of local objects. If it is found that an operation occurred on an out-of-date object and the change is not consistent, the change to the old object has to be rolled back, since the authoritative object's state is always accepted.
