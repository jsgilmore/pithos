//
// Generated file, do not edit! Created by opp_msgc 4.1 from applications/Pithos/Pithos.msg.
//

#ifndef _PITHOS_M_H_
#define _PITHOS_M_H_

#include <omnetpp.h>

// opp_msgc version check
#define MSGC_VERSION 0x0401
#if (MSGC_VERSION!=OMNETPP_VERSION)
#    error Version mismatch! Probably this file was generated by an earlier version of opp_msgc: 'make clean' should help.
#endif

// cplusplus {{
#include <TransportAddress.h>
// }}



/**
 * Enum generated from <tt>applications/Pithos/Pithos.msg</tt> by opp_msgc.
 * <pre>
 * enum PayloadTypes
 * {
 *     WRITE = 1;		
 *     OVERLAY_WRITE_REQ = 2;
 *     INFORM = 3;		
 *     INFORM_REQ = 4;	
 * };
 * </pre>
 */
enum PayloadTypes {
    WRITE = 1,
    OVERLAY_WRITE_REQ = 2,
    INFORM = 3,
    INFORM_REQ = 4
};

/**
 * Class generated from <tt>applications/Pithos/Pithos.msg</tt> by opp_msgc.
 * <pre>
 * packet PithosMsg {
 *     int value;
 *     int payloadType enum(PayloadTypes);
 *     TransportAddress sourceAddress;
 *     TransportAddress destinationAddress;
 * }
 * </pre>
 */
class PithosMsg : public ::cPacket
{
  protected:
    int value_var;
    int payloadType_var;
    ::TransportAddress sourceAddress_var;
    ::TransportAddress destinationAddress_var;

    // protected and unimplemented operator==(), to prevent accidental usage
    bool operator==(const PithosMsg&);

  public:
    PithosMsg(const char *name=NULL, int kind=0);
    PithosMsg(const PithosMsg& other);
    virtual ~PithosMsg();
    PithosMsg& operator=(const PithosMsg& other);
    virtual PithosMsg *dup() const {return new PithosMsg(*this);}
    virtual void parsimPack(cCommBuffer *b);
    virtual void parsimUnpack(cCommBuffer *b);

    // field getter/setter methods
    virtual int getValue() const;
    virtual void setValue(int value_var);
    virtual int getPayloadType() const;
    virtual void setPayloadType(int payloadType_var);
    virtual TransportAddress& getSourceAddress();
    virtual const TransportAddress& getSourceAddress() const {return const_cast<PithosMsg*>(this)->getSourceAddress();}
    virtual void setSourceAddress(const TransportAddress& sourceAddress_var);
    virtual TransportAddress& getDestinationAddress();
    virtual const TransportAddress& getDestinationAddress() const {return const_cast<PithosMsg*>(this)->getDestinationAddress();}
    virtual void setDestinationAddress(const TransportAddress& destinationAddress_var);
};

inline void doPacking(cCommBuffer *b, PithosMsg& obj) {obj.parsimPack(b);}
inline void doUnpacking(cCommBuffer *b, PithosMsg& obj) {obj.parsimUnpack(b);}


#endif // _PITHOS_M_H_
